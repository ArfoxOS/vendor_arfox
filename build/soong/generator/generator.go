...
func init() {
	android.RegisterModuleType("arfox_generator", GeneratorFactory)
}
...
func (g *Module) GenerateAndroidBuildActions(ctx android.ModuleContext) {
	...
	if depRoot == "" {
		...
		depRoot = arfoxExpandVariables(ctx, depRoot)
	}

	// Glob dep_files property
	for _, dep_file := range g.properties.Dep_files {
		dep_file = arfoxExpandVariables(ctx, dep_file)
	...
	cmd := arfoxExpandVariables(ctx, String(g.properties.Cmd))
