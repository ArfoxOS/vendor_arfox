...
function check_product()
{
    ...
    if (echo -n $1 | grep -q -e "^arfox_") ; then
        ARFOX_BUILD=$(echo -n $1 | sed -e 's/^arfox_//g')
    else
        ARFOX_BUILD=
    fi
    export ARFOX_BUILD
...
function breakfast()
{
    target=$1
    local variant=$2
    source ${ANDROID_BUILD_TOP}/vendor/arfox/vars/arfox_target_release
    ...
    if [ $# -eq 0 ]; then
        ...
        if [[ "$target" =~ -(user|userdebug|eng)$ ]]; then
            ...
            # This is probably just the Arfox model name
            if [ -z "$variant" ]; then
                variant="userdebug"
            fi

            lunch arfox_$target-$arfox_target_release-$variant
        fi
    fi
    return $?
}
...
{
    if [ "$OUT" ] ; then
        ZIPPATH=`ls -tr "$OUT"/ArfoxOS-*.zip | tail -1`
        if [ ! -f $ZIPPATH ] ; then
            echo "Nothing to eat"
            return 1
        fi
        ...
        echo "Found device"
        if (adb shell getprop ro.arfox.device | grep -q "$ARFOX_BUILD"); then
            echo "Rebooting to sideload for install"
            adb reboot sideload-auto-reboot
            adb wait-for-sideload
            adb sideload $ZIPPATH
        else
            echo "The connected device does not appear to be $ARFOX_BUILD, run away!"
        fi
        return $?
    else
        echo "Nothing to eat"
...
function githubremote()
{
    ...
    git remote add github https://github.com/ArfoxOS/$PROJECT
...
function installboot()
{
    ...
    adb root
    adb wait-for-device-recovery
    if (adb shell getprop ro.arfox.device | grep -q "$ARFOX_BUILD");
    then
...
function dopush()
{
    ...
    if (adb shell getprop ro.arfox.device | grep -q "$ARFOX_BUILD") || [ "$FORCE_PUSH" = "true" ];
    then
...
function repopick() {
    T=$(gettop)
    $T/vendor/arfox/build/tools/repopick.py $@
}
