...
arfox_local_manifest = ".repo/local_manifests/snippets/arfox.xml"
arfox_default_revision = "15"
arfox_dependencies = "arfox.dependencies"
org_manifest = "arfox"  # leave empty if org is provided in manifest
org_display = "ArfoxOS"  # needed for displaying

default_manifest = ".repo/manifests/default.xml"
arfox_manifest = ".repo/manifests/snippets/arfox.xml"
...
def get_revision(manifest=None, p="build"):
    return arfox_default_revision

def get_from_manifest(device_name):
    if os.path.exists(arfox_local_manifest):
        man = load_manifest(arfox_local_manifest)
...
def is_in_manifest(project_path):
    for local_path in load_manifest(arfox_local_manifest).findall("project"):
...
def add_to_manifest(repos, fallback_branch=None):
    lm = load_manifest(arfox_local_manifest)
    mlm = load_manifest(default_manifest)
    arfoxm = load_manifest(arfox_manifest)
    ...
    for repo in repos:
        ...
        if 'branch' in repo:
            ...
            repo_branch=arfox_default_revision
        ...
        if exists_in_tree(arfoxm, repo_path) != None:
            existing_m_project = exists_in_tree(arfoxm, repo_path)
...
def fetch_dependencies(repo_path, fallback_branch=None):
    ...
    dep_p = '/'.join((repo_path, arfox_dependencies))
...
def detect_revision(repo):
    ...
    if has_branch(result, arfox_default_revision):
        print("Falling back to arfox revision: %s"
              % arfox_default_revision)
        return arfox_default_revision
